...
import { GroupedFilterDataInstance, MEData, SimpleFilterData } from 'src/app/shared/application_data';
...
export class GroupFilterComponent implements OnInit {
  ...
  public data_search: ReplaySubject<GroupedFilterDataInstance[]> = new ReplaySubject<GroupedFilterDataInstance []>(1);
  ...
  protected filterMulti() {
    ...
    this.data_search.next(
      ...
      GroupsCopy.filter(g => {
        ...
        if (!showGroup) {
          g.group_data = g.group_data.filter(g1 => g1.toLowerCase().indexOf(search) > -1);
        }
  ...
  myData(){
    ...
    this.data = this.dataService.group_filter_data.get(this.uuid)?.data as GroupedFilterDataInstance[]
